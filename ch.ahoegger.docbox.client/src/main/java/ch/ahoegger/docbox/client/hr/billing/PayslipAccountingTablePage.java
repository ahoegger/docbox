...
/**
 * <h3>{@link PayslipAccountingTablePage}</h3>
 *
 * @author Andreas Hoegger
 */
@PageData(PayslipAccountingTableData.class)
public class PayslipAccountingTablePage extends AbstractDocboxPageWithTable<PayslipAccountingTablePage.Table> {
  ...
  protected void execLoadData(SearchFilter filter) {
    importPageData(BEANS.get(IPayslipAccountingService.class).getTableData((PayslipAccountingSearchFormData) filter.getFormData()));
  }

  @Override
  protected Class<? extends ISearchForm> getConfiguredSearchForm() {
    return PayslipAccountingSearchForm.class;
  }

  @Override
  protected void execInitPage() {
    registerDataChangeListener(IPayslipAccountingEntity.ENTITY_KEY);
  }

  @Override
  protected IPage<?> execCreateChildPage(ITableRow row) {
    EntityTablePage page = new EntityTablePage(getPartnerId());
    page.setPayslipAccountingId(getTable().getIdColumn().getValue(row));
    return page;
  }

  @Override
  public PayslipAccountingSearchForm getSearchFormInternal() {
    return (PayslipAccountingSearchForm) super.getSearchFormInternal();
  }
  ...
  protected void execInitSearchForm() {
    ...
    if (getPartnerId() != null) {
      getSearchFormInternal().getPartnerField().setValue(getPartnerId());
      getSearchFormInternal().getPartnerField().setEnabled(false);
    }
  ...
  public class Table extends AbstractTable {
    ...
    public class NewPayslipMenu extends AbstractMenu {
      ...
      protected void execAction() {
        ...
        BigDecimal payslipAccountingId = getIdColumn().getValue(getTable().getSelectedRow());
        PayslipAccountingForm form = new PayslipAccountingForm();
        form.getPartnerField().setValue(partnerId);
        form.setPayslipAccountingId(payslipAccountingId);
    ...
    @Order(50500)
    public class EditPayslipAccountingMenu extends AbstractMenu {
      @Override
      protected String getConfiguredText() {
        return TEXTS.get("EditPayslip");
      }
    ...
    @Order(51000)
    public class DeletePayslipAccountingMenu extends AbstractMenu {
      @Override
      protected String getConfiguredText() {
        return TEXTS.get("DeletePayslip");
      }
      ...
      protected void execAction() {
        if (MessageBox.YES_OPTION == MessageBoxes.createYesNo()
          ...
          BEANS.get(IPayslipAccountingService.class).delete(getIdColumn().getSelectedValue());

          getDesktop().dataChanged(IPayslipAccountingEntity.ENTITY_KEY);
        }
      }
    }
