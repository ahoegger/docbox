...
public class UserServiceTest extends AbstractTestWithDatabase {
  ...
  public void setupDb() throws Exception {
    ...
    BEANS.get(UserTableTask.class).createUser(sqlService, "adminstrator", "first.admin", userId01, "secret", true, true);
    BEANS.get(UserTableTask.class).createUser(sqlService, "semil", "borak", userId02, "secret", true, false);
    BEANS.get(UserTableTask.class).createUser(sqlService, "inactive.name", "inactive.firstname", userId03, "inactive.secret", false, false);
    BEANS.get(UserTableTask.class).createUser(sqlService, "modify.name", "modify.firstname", userId04, "modify.secret", true, false);
    BEANS.get(UserTableTask.class).createUser(sqlService, "modifyPwd.name", "modifyPwd.firstname", userId05, "modifyPwd.secret", true, false);
  }
  ...
  public void testModifyUserPassword() {
    ...
    Assert.assertTrue(BEANS.get(SecurityService.class).authenticate(userId05, "modifyPwd.secret".toCharArray()));
    ...
    // try to login
    Assert.assertFalse(BEANS.get(SecurityService.class).authenticate(userId05, "modifyPwd.secret".toCharArray()));
  ...
  public void testLogin() {
    Assert.assertTrue(BEANS.get(SecurityService.class).authenticate(userId02, "secret".toCharArray()));
    Assert.assertFalse(BEANS.get(SecurityService.class).authenticate(userId03, "inactive.secret".toCharArray()));
  }
}
