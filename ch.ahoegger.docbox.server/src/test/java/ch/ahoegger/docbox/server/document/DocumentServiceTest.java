...
public class DocumentServiceTest extends AbstractTestWithDatabase {
  ...
  public void setupDb() throws Exception {
    ...
    BEANS.get(CategoryTableTask.class).insert(sqlService, categoryId01, "sampleCategoriy", null, documentDate, null);

    // partner
    BEANS.get(PartnerTableTask.class).insert(sqlService, partnerId01, "partner01", null, cal.getTime(), null);
    BEANS.get(PartnerTableTask.class).insert(sqlService, partnerId02, "partner02", null, cal.getTime(), null);

    // conversation
    BEANS.get(ConversationTableTask.class).insert(sqlService, conversationId01, "con01", null, cal.getTime(), null);
    BEANS.get(ConversationTableTask.class).insert(sqlService, conversationId02, "con02", null, cal.getTime(), null);
