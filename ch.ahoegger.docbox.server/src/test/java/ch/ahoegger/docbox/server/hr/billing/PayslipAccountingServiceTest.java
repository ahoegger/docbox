...
/**
 * <h3>{@link PayslipAccountingServiceTest}</h3>
 *
 * @author Andreas Hoegger
 */
public class PayslipAccountingServiceTest extends AbstractTestWithDatabase {
  ...
  private BigDecimal payslipAccountingGroupId = BEANS.get(IdGenerateService.class).getNextIdBigDecimal();
  ...
  protected void execSetupDb(Connection connection) throws Exception {
    ...
    BEANS.get(PayslipAccountingService.class).insert(connection, payslipAccountingGroupId, partnerId, null, documentId, "Dezember ",
    ...
    BEANS.get(EntityService.class).insert(connection, expenceId, partnerId, payslipAccountingGroupId, ExpenseCode.ID, LocalDateUtility.toDate(LocalDate.now().minusDays(2)), null, BigDecimal.valueOf(23), "desc");
    BEANS.get(EntityService.class).insert(connection, workId, partnerId, payslipAccountingGroupId, WorkCode.ID, LocalDateUtility.toDate(LocalDate.now().minusDays(2)), BigDecimal.valueOf(3), null, "desc");
  ...
  public void testDelete() {
    ...
    BEANS.get(PayslipAccountingService.class).delete(payslipAccountingGroupId);

    Assert.assertFalse(BEANS.get(DocumentStoreService.class).exists(docData.getDocumentPath()));
    // unbilled group still exists
    Assert.assertEquals(0, BEANS.get(PayslipAccountingService.class).getTableData(new PayslipAccountingSearchFormData()).getRowCount());
    ...
    sd.setPayslipAccountingId(payslipAccountingGroupId);
    Assert.assertEquals(0, BEANS.get(EntityService.class).getEntityTableData(sd).getRowCount());
    sd.setPayslipAccountingId(UnbilledCode.ID);
