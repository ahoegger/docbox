...
public class PostingGroupServiceTest_PrepareCreate extends AbstractTestWithDatabase {
  ...
  public void setupDb() throws Exception {
    ...
    // partner
    BEANS.get(PartnerService.class).insert(sqlService.getConnection(), partnerId01, "Homer Simpson", null, LocalDateUtility.toDate(today.minusDays(20)), null);

    // employee
    BEANS.get(EmployeeService.class).insert(sqlService.getConnection(), partnerId01, "Homer", "Simpson", "Nashvill Street 12a", "Santa Barbara CA-90051", "ahv123564789", "iban987654321",
    ...
    // create Tax groups
    BEANS.get(TaxGroupService.class).insert(sqlService.getConnection(), taxGroupId01, "", LocalDateUtility.toDate(today.minusYears(1).withDayOfYear(1)),
        LocalDateUtility.toDate(today.minusYears(1).withDayOfYear(today.minusYears(1).lengthOfYear())));
    BEANS.get(TaxGroupService.class).insert(sqlService.getConnection(), taxGroupId02, "", LocalDateUtility.toDate(today.withDayOfYear(1)),
        LocalDateUtility.toDate(today.withDayOfYear(today.lengthOfYear())));
    BEANS.get(TaxGroupService.class).insert(sqlService.getConnection(), taxGroupId03, "", LocalDateUtility.toDate(today.plusYears(1).withDayOfYear(1)),
        LocalDateUtility.toDate(today.plusYears(1).withDayOfYear(today.plusYears(1).lengthOfYear())));

    // create entities
    BEANS.get(EntityService.class).insert(sqlService.getConnection(), entityId01, partnerId01, UnbilledCode.ID, WorkCode.ID, LocalDateUtility.toDate(LocalDate.of(today.getYear(), 3, 31)), BigDecimal.valueOf(2), null, "desc");
    BEANS.get(EntityService.class).insert(sqlService.getConnection(), entityId02, partnerId01, UnbilledCode.ID, WorkCode.ID, LocalDateUtility.toDate(LocalDate.of(today.getYear(), 4, 1)), BigDecimal.valueOf(2), null, "desc");
    BEANS.get(EntityService.class).insert(sqlService.getConnection(), entityId03, partnerId01, UnbilledCode.ID, WorkCode.ID, LocalDateUtility.toDate(LocalDate.of(today.getYear(), 4, 13)), BigDecimal.valueOf(2), null, "desc");
    BEANS.get(EntityService.class).insert(sqlService.getConnection(), entityId04, partnerId01, UnbilledCode.ID, WorkCode.ID, LocalDateUtility.toDate(LocalDate.of(today.getYear(), 4, 30)), BigDecimal.valueOf(2), null, "desc");
    BEANS.get(EntityService.class).insert(sqlService.getConnection(), entityId05, partnerId01, UnbilledCode.ID, WorkCode.ID, LocalDateUtility.toDate(LocalDate.of(today.getYear(), 5, 1)), BigDecimal.valueOf(2), null, "desc");
